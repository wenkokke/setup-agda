...
import { agdaComponents, BuildOptions, BundleOptions } from '../types.js'
...
export async function icuConfigureOptions(
  ...
  // Add --extra-prog-path
  for (const msysPath in msys.paths)
...
const libname = platform === 'win32' ? 'icu-io' : 'icu-i18n'
  ...
  switch (platform) {
    ...
    case 'darwin': {
      ...
      for (const [libName, depNames] of libsAndDepsToChange) {
        ...
        const libTo = path.join(dest, libNameTo)
        for (const depName of depNames) {
